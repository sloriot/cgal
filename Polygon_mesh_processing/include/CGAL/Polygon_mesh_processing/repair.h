...
///\todo why do we use the point rather than the vertex
...
template <class TriangleMesh, class halfedge_descriptor, class face_descriptor>
                                        ...
                                        bool verbose)
{
  ...
  if (!one_halfedge_per_border.empty()){
    ...
    if (verbose)
      std::cout << "INFO: Nothing to correct\n";
  }

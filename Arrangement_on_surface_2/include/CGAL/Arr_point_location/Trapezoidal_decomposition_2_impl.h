...
//            based on the methods implemented by Oren Nechushtan
                          ...
                          Halfedge_const_handle he)
{
  ...
  if (last_cv == curr_item) last_cv = vtx_item;
  if (prev_cv == curr_item) prev_cv = vtx_item;
#endif
...
undo_split_trapezoid_by_vertex(Dag_node& tr_node, const Curve_end& ce)
{
  ...
  //CGAL_assertion(traits->is_active(tr_left_node.get_data()) &&
  //               traits->is_active(tr_right_node.get_data()));
  ...
  deactivate_trapezoid( tr_right_node, &tr_left_node);
  //tr_right_node->remove(&tr_left_node);
  update_largest_leaf_depth( tr_left_node.depth());
  //tr_left_node is not an inner node
  // mark root as deleted
  deactivate_vertex(tr_node); //tr_node->remove();
  // no need to update m_number_of_dag_nodes because the number of nodes does
  // not change.
                            ...
                            Halfedge_const_handle he)
{
  ...
  Dag_node top_node(Td_active_trapezoid(split_tr.left(), split_tr.right(), he,
                                        split_tr.top()));
  ...
  Dag_node bottom_node(Td_active_trapezoid(split_tr.left(),split_tr.right(),
                                           split_tr.bottom(), he));
                         //Td_active_trapezoid::TD_TRAPEZOID,
                         //split_tr.on_boundaries_flag() &
                         // (CGAL_TD_ON_LEFT_BOUNDARY  |
...
#ifndef CGAL_NO_TRAPEZOIDAL_DECOMPOSITION_2_OPTIMIZATION
  if (last_cv == curr_item) last_cv = sep;
  if (prev_cv == curr_item) prev_cv = sep;
#endif
                              ...
                              CGAL_precondition_code(lt))
{
  ...
  if (he->source() == v)
    // if ((ce_pair.second == ARR_MIN_END &&
    //     he->direction() == ARR_LEFT_TO_RIGHT) ||
    //     (ce_pair.second == ARR_MAX_END &&
    //      he->direction() == ARR_RIGHT_TO_LEFT)  )
    return *split_trapezoid_by_vertex(*node, v, he);
  else return *split_trapezoid_by_vertex(*node, v, he->twin());
}
    ...
    if (traits->is_td_vertex(curr_item)) {
      ...
      //         (!is_fict_vtx && traits->equal_2_object()(boost::apply_visitor(point_for_vertex_visitor(), curr_item), p)) )
        ...
        if (he == m_empty_he_handle) {
          ...
          if (up == EQUAL) {
            // point found!
            if (traits->is_active(curr_item)) return POINT;
            curr_node = curr_node.left_child();
          }
          else if (up == LARGER) {          // vertical ray shut up
            curr_node = curr_node.right_child();
          }
          else curr_node = curr_node.left_child();
          continue;
        }
        else {
          // he was given
          bool is_equal_to_he_min =
            traits->equal_curve_end_2_object()(Curve_end(he,ARR_MIN_END), p);
          bool is_equal_to_he_max =
      ...
      if (cres == SMALLER) {
        ...
        if (he == m_empty_he_handle) {
          ...
          if ((up == EQUAL) || traits->is_vertical(curr_item)) {
            if (traits->is_active(curr_item)) return CURVE;
            curr_node = curr_node.left_child();
          }
          else if (up == LARGER) {
            curr_node = curr_node.right_child();
          ...
          bool is_min_equal =
            traits->equal_curve_end_2_object()(Curve_end(he,ARR_MIN_END),
                                               Curve_end(he_cv,ARR_MIN_END));

          bool is_max_equal =
            traits->equal_curve_end_2_object()(Curve_end(he,ARR_MAX_END),
                                               Curve_end(he_cv,ARR_MAX_END));

          CGAL_warning (is_min_equal || is_max_equal);
          CGAL_USE(is_max_equal);

          Comparison_result res = is_min_equal ?
          ...
          switch(res) {
           case LARGER: curr_node = curr_node.right_child(); break;
           case SMALLER: curr_node = curr_node.left_child(); break;
           case EQUAL:
            switch(up) {
             case LARGER: curr_node = curr_node.right_child(); break;
             case SMALLER: curr_node = curr_node.left_child(); break;
    ...
    if (traits->is_td_vertex(curr_item)) {
      ...
      //         (!is_fict_vtx &&  traits->equal_curve_end_2_object()(boost::apply_visitor(point_for_vertex_visitor(), curr_item), ce)) )
        ...
        if (!p_cv) {
          ...
          else if (up == LARGER) curr_node = curr_node.right_child();
          else curr_node = curr_node.left_child();
          continue;
        }
        else {
          // p_cv was given
          bool is_equal_to_he_min =
            traits->equal_curve_end_2_object()(Curve_end(*p_cv,ARR_MIN_END), ce);
          bool is_equal_to_he_max =
            traits->equal_curve_end_2_object()(Curve_end(*p_cv,ARR_MAX_END), ce);
      ...
      if (cres == SMALLER) {
        ...
        if (!p_cv) {
               ...
               (ce.cv(), ce.ce()) == ARR_INTERIOR) )
          {
            ...
            bool is_min_equal =
              traits->equal_curve_end_2_object()(Curve_end(*p_cv,ARR_MIN_END),
                                                 Curve_end(he_cv,ARR_MIN_END));

            bool is_max_equal =
              traits->equal_curve_end_2_object()(Curve_end(*p_cv,ARR_MAX_END),
                                                 Curve_end(he_cv,ARR_MAX_END));
    ...
    if (traits->is_td_vertex(curr_item)) {
      ...
      //         (!is_fict_vtx &&  traits->equal_2_object()(boost::apply_visitor(point_for_vertex_visitor(), curr_item), p)) )
        ...
        if (!p_cv) {
          ...
          // p_cv was given
          bool is_equal_to_he_min =
            traits->equal_curve_end_2_object()(Curve_end(*p_cv,ARR_MIN_END), p);
          bool is_equal_to_he_max =
            traits->equal_curve_end_2_object()(Curve_end(*p_cv,ARR_MAX_END), p);
...
container2dag(Nodes_map& ar, int left, int right, int& num_of_new_nodes) const
{
  ...
  if (right > left) {
    ...
    if (is_interior) {
      curr_node.replace(Td_map_item(Td_active_vertex(v, m_empty_he_handle)),
...
is_last_edge(Halfedge_const_handle /* he */ , Td_map_item& vtx_item)
{
  ...
  //if he is the only halfedge around v -> return true
...
template <typename Td_traits>
void Trapezoidal_decomposition_2<Td_traits>::remove(Halfedge_const_handle he)
{
  //print_dag_addresses(*m_dag_root);
  ...
  do {
    ...
    inc_btm =
      is_end_point_left_low(Curve_end(traits->vtx_to_ce(btm_it_tr.right())),
    ...
    last_new_tr_item = new_node.get_data();
    //new_node.operator->(); //get the last new trapezoid created
    ...
    if (prev_inc_btm != inc_btm) {
      ...
      m_number_of_dag_nodes += num_of_new_nodes;
      //new vertex nodes (rooted at tmp) were added
                   ...
                   const bool up_direction /*=true*/) const
{
  ...
  lt = search_using_dag(curr, traits, p, m_empty_he_handle, up_direction ?
  ...
  if (traits->is_td_trapezoid(item)) {
    ...
    */
                                              ...
                                              traits->vtx_to_ce(tr.right())))))
    {
      ...
      return up_direction ?
        locate(traits->vtx_to_ce(tr.right()),lt) :
        locate(traits->vtx_to_ce(tr.left()),lt);
    }
           ...
           const X_monotone_curve_2& cv)
{
  ...
  Halfedge_const_handle merged_he =
    (traits->equal_curve_end_2_object()(Curve_end(cv1, ARR_MIN_END),
                                        Curve_end(cv2, ARR_MAX_END)) ||
     traits->equal_curve_end_2_object()(Curve_end(cv1, ARR_MIN_END),
                                        Curve_end(cv2, ARR_MIN_END))) ?
    he1->twin() :  he1;
  ...
  CGAL_precondition(is_end_point_left_low(Curve_end(cv, ARR_MIN_END), ce));
  CGAL_precondition(is_end_point_right_top(Curve_end(cv, ARR_MAX_END), ce));
#endif
  ...
  Vertex_const_handle leftmost_v  =
    (left_he->direction() == ARR_LEFT_TO_RIGHT) ?
    left_he->source() : left_he->target();
  Vertex_const_handle rightmost_v =
    (right_he->direction() == ARR_LEFT_TO_RIGHT) ?
    right_he->target() : right_he->source();
  ...
  CGAL_assertion(traits->is_active(above_cv_left) &&
                 traits->is_active(above_cv_right) );
  CGAL_assertion(traits->is_active(below_cv_left) &&
                 traits->is_active(below_cv_right) );
                              ...
                              Dag_node& node)
{
  ...
  if (traits->is_td_trapezoid(node.get_data())) return 1;
  ...
  return (1 + (std::max)(longest_query_path_length_rec(minus_inf, min_node,
                                                       false, new_max_node,
                                                       node.left_child()) ,

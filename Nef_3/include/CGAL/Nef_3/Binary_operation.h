...
class Binary_operation : public CGAL::SNC_decorator<Map> { 
 ...
 public:
  ...
  Vertex_handle binop_local_views( Vertex_const_handle v0, Vertex_const_handle v1,
				   ...
				   ,Association& A, bool simplify_redundant_edges)
    ...
    O.subdivide( &*v0, &*v1, A);
    O.select( BOP);
    O.simplify(A, simplify_redundant_edges);
  ...
  public:
    Intersection_call_back( SNC_structure& s0, SNC_structure& s1,
      ...
      void operator()(Halfedge_handle e0, Object_handle o1, const Point_3& ip, bool simplify_redundant_edges)
      const {
	...
	if( inverse_order)
	  std::swap( v0, v1);
	D.binop_local_views( v0, v1, bop, result,A,simplify_redundant_edges);
      ...
      \precondition: the structure |result| is empty.}*/ 
  {
	...
	binop_local_views( v0, v, BOP, *this->sncp(),A,simplify_redundant_edges);
	ignore[v] = true;
      }
      else if( CGAL::assign( e, o)) {
	CGAL_NEF_TRACEN("p0 found on edge");
	Vertex_handle v1 = create_local_view_on( p0, e);
	binop_local_views( v0, v1, BOP, *this->sncp(),A,simplify_redundant_edges);
	...
	binop_local_views( v1, v0, BOP, *this->sncp(),A,simplify_redundant_edges);
	this->sncp()->delete_vertex(v1);
      } 
      else if( CGAL::assign( f, o)) {
